...
\item Nengo-Loihi 0.5.0~\citep{blouw2018a}; to compile Nengo models onto the Loihi emulator and hardware (courtesy of ABR),
...
Experiments involving the methods of \citet{boerlin2013predictive} in Nengo are currently unpublished (provided by Dan Rasmussen).
...
In addition, it is used to construct RC, FORCE, full-FORCE, and Nengo networks---each in spiking and non-spiking mode, and in an analagous manner to one another---in order to facilitate meaningful comparisons between these architectures.
Another useful aspect of the software is that it provides a unified toolkit for constructing and analyzing linear time-invariant systems, in a number of representations (state-space, transfer function, zero-pole-gain), while being compatible with the synapses of Nengo.
This unifies the language used to both build, describe, and analyze LTI networks and synapses.
We now summarize some of the remaining features of this software that are used throughout.
...
In general, we find that each method of normalizing state-space models typically improves the robustness of our networks across a wide range of parameters, but one is never strictly better than all others in every situation.
...
Many common routines in Nengo employ the use of high-dimensional random sampling~\citep{voelker2017}, in particular the uniform sampling of encoders $\V{e}_i$ from the surface of the hypersphere, and the uniform sampling of evaluation points from $S$ (typically the interior of the hyperball).
