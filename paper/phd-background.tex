...
Recurrent neural networks are notoriously difficult to train and interpret in practice.
...
The First-Order Reduced and Controlled Error~\citep[FORCE;][]{sussillo2009generating} method improves the performance of RC networks, by simultaneously re-encoding the learned output back into the reservoir. 
...
Full-FORCE~\citep{depasquale2018full} solves the latter problem, by allowing the desired state-vector to be encoded into the network in the form of ``hints'' during training, but does so without accounting for the dynamics introduced by the synaptic $\tau$, as done by the NEF in Principle~3.
...
Learning these dimensions is a somewhat separate matter; the NEF does not \emph{limit} itself to situations where the dynamics are known apriori -- techniques from system identification~\citep{nelles2013nonlinear} can be applied, the optimization can be applied in the time-domain to a raw teaching signal without needing closed-form equations for the dynamics~\citep{duggins2017incorporating}, or backpropagation can be applied~\citep{rasmussen2018nengodl}.
Likewise, random feedback and FORCE learning can also be applied, in which case it becomes essentially the same as FORCE learning (details below).
The NEF embraces all such methods, but makes a preference for those that are efficient and amenable to analysis.
However, we find the approach of introducing random high-dimensional feedback to often unnecessary be unnecessary for our purposes (section~\ref{sec:delay-rc}), and furthermore re-encoding the target output into the network as a training signal can lead to a loss in accuracy in simple cases (section~\ref{sec:force-comparison}).
...
In the case of RC networks, they make a conceptual separation between a fixed {\it reservoir} of randomly-connected neural units, and a learned {\it readout}.
