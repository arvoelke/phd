...
The basic recipe is conceptually simple: obtain one of the many popular off-the-shelf toolboxes for deep learning, pick your favourite network architecture, set its hyperparameters, and then throw as much training data at the network as your CPUs or GPUs can handle.
...
However, the opacity of its optimization procedure comes as a double-edged sword: while it is easy to apply deep learning to many problems with minimal hand-engineering, it becomes unclear what effect most hyperparameter changes will have on overall performance~\citep{bergstra2012random}.
For example, it has been observed that using a rectified linear~(ReLU) activiation unit for each nonlinearity can dramatically reduce training times on a variety of benchmarks~\citep{glorot2011deep}, yet this observation still does not have any theoretically-grounded explanation that could inform the optimal selection of nonlinearities or related hyperparameters.

Despite its breakthroughs, the field is well-aware that a feed-forward architecture is incapable of learning temporal relationships in the input data, which is necessary for tasks involving video, speech, and language understanding with long-range dependencies~\citep{bengio1994learning}.
...
The next largest (non-NEF) RNN that we can find with published resource counts is a ``large scale'' speech recognition example from Google, consisting of \numprint{6800} units in its largest instantiation~\citep[][Table~1]{sak2014long} -- on the order of \numprint{e7} times smaller than the human brain.
...
Results on a chaotic time-series prediction benchmark suggest that interleaving LSTMs together with our novel memory cell, trained via BPTT, greatly improve an RNN's ability to integrate nonlinear delay-differential equations.
...
This method and architecture are referred to as the Echo State Network~\citep[ESN;][]{jaeger2001echo}, or Liquid State Machine~\citep[LSM;][]{maass2002real}, depending on whether the neural nonlinearities are static or spiking models, respectively.
The intuition is that the randomized feedback, when initialized in an appropriate manner, causes traces of the input to dynamically reverberate, or ``echo'', throughout the state of the network; a common metaphor is depicted by throwing rocks into a bucket of water and observing the ripples interact through time.\footnote{To demonstrate the relevance of this metaphor, \citet{fernando2003pattern} went so far as to actually implement an LSM using a bucket of water.}
These traces can then be decoded to reconstruct the output signal, thus approximating a nonlinear transfer function of the input signal.

There are two great insights driving the adoption of RC: (1) randomized feedback provides an overcomplete basis of nonlinear time-varying signals that can be linearly combined to approximate some output signal, and (2) by leaving the recurrent weights fixed, the optimization procedure is greatly simplified; training the readout weights amounts to a least-squares problem.
Additional details are provided in section~\ref{sec:relationships} after introducing the NEF.
In section~\ref{sec:delay-rc}, we compare the performance of RC to the NEF on a delay-line benchmark.

\subsection{Engineered Approaches}

A very different set of approaches to training RNNs is rooted in the application of engineering and control-theoretic methods to modelling neurobiological systems~\citep{eliasmith1999developing}.
...
That is, rather than supposing our only knowledge is raw input-output data, in many real-world scenarios we have some model of the dynamical system that actually generated the data.
...
Often times, the dynamical system is simply a description of a particular computation that we know to be useful, such as a controller for a robotic arm, or the examples given in section~\ref{sec:dynamics-language}.

Instances of this approach include the Neural Engineering Framework~\citep[NEF;][]{eliasmith2003a, duggins2017incorporating}, first-order reduced and controlled error~\citep[FORCE;][]{sussillo2009generating, nicola2016supervised, depasquale2018full}, balanced spiking networks~\citep{boerlin2013predictive, thalmeier2016learning, alemi2018learning}, and relatives~\citep{jaeger2014controlling, gilra2017predicting}.
Reviews of many of these methods are given by \citet{deneve2016efficient} and \citet{abbott2016building}.

It is important to note that, although all of these approaches are engineered from a top-down perspective, there is no reason BPTT (or any other optimization method) cannot also be applied to fine-tune the resulting RNN end-to-end from example input-output pairs -- a hybrid approach that is embraced by the NEF and its software stack~\citep{rasmussen2018nengodl}.
The point is that prior knowledge of the task can help place appropriate constraints on the initial state of the RNN, and provide transparency into the relationships between network parameters, configuration, and function.
As we will show, such transparency enables theoretical insights into neural computation, and practical advances on RNN benchmarks, that would otherwise be quite difficult to obtain with a pure black-box approach.

In section~\ref{sec:nef} we provide a detailed account of the NEF, and in section~\ref{sec:relationships} we draw comparisons to FORCE.
In essence, full-FORCE~\citep{depasquale2018full} can be viewed an an instance of the NEF where the learning is applied online rather than offline, and randomized feedback is thrown into the equation for the same purposes as in RC.
Similarly, \citet{jaeger2014controlling} can be viewed as a variation of the NEF~\citep{aubin2017}.

Balanced spiking networks share many commonalities with the NEF, primarily a procedure for mapping a dynamical state vector onto a population of spiking neurons, via a low-rank weight matrix that takes into account the dynamical models of the neurons and synapses.
...
However, this scheme relies on assumptions of near-instantaneous communication between all neurons~\citep{boerlin2013predictive}, linear neurons with minimal leakage and no refractory period, and nonlinearities in the synapses to implement nonlinear dynamics~\citep{thalmeier2016learning}.
It is unclear how the scaling holds up when these assumptions are violated, which is of great importance when considering the targets of modelling biological systems and mapping onto neuromorphic hardware.
As outlined in section~\ref{sec:nef-suitability}, the NEF is correct under fairly weak assumptions, which leads to it being robust and extensibile.
These key factors are what contribute to it being our toolkit of choice when it comes to training RNNs for the neuromorphic hardware summarized in section~\ref{sec:neuromorphic}.
