...
The state-of-the-art in neuromorphic computing is on the verge of reaching this level of performance, with Braindrop, Loihi, and TrueNorth~\citep{merolla2014million}, all coming within a factor of \numprint{e1}--\numprint{e3} for roughly equivalent measures of energy per synaptic operation and typical network configurations~\citep{braindrop2019}.
Such potential energy savings are not only tremendous, but a prerequisite to reach the exascale level of computation achieved by the human brain, unless one has access to a nuclear power plant producing on the order of \numprint{e9}\ watts~\citep{furber2012build, neurogrid2014}.
However, such power measurements are not straight-forward to interpret, as there are many other factors such as the cost of scaling, flexibility of the hardware, and open questions surrounding whether the number of synaptic events is the best metric for evaluating system performance, and how much biological detail is necessary~\citep{eliasmith2013build}--all of which make fair benchmarking an incredibly challenging problem~\citep{stewart2015closed}.
...
Yet, there are many common features between neuromorphic hardware architectures, including: parallelism, memory colocated with computational elements, heterogeneity and sparsity, structured connectivity, minimization of spike traffic and synaptic events, and real-time interfacing.
...
This chip was designed explicitly with the NEF in mind~\citep{boahen2017neuromorph}, by way of a novel mapping~\citep{voelker2017iscas, neckar2018optimizing}, accompanied by a number of inventions including temperature-invariance~\citep{abrams2017, reidpint2019, benjamintemp2019}, sparse encoding~\citep{fokserial2018}, and spike-thinning~\citep{fokthinning2019}.

To put this achievement into perspective, we begin with an overview of its predecessor, Neurogrid~\citep{neurogrid2014}. 
