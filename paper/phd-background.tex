...
One of the central challenges in computational neuroscience involves improving our understanding of how dynamic stimuli can be processed by neural mechanisms to drive behaviour.
...
The NEF has been used to construct a wide variety of neural models, including a 6.6~million neuron functioning model of the human brain, named ``Spaun'', that is capable of performing perceptual, motor, and cognitive tasks~\citep{eliasmith2012, choo2018}.
...
The flexibility of the NEF has lead to it being deployed on mixed-analog-digital neuromorphic chips~\citep{choudhary2012silicon, corradi2014, voelker2017iscas, voelker2017neuromorphic, braindrop2019} and digital architectures~\citep{bekolay2013, wang2014compact, mundy2015, berzish2016, wang2017neuromorphic, blouw2018a}.
...
Chapter~\ref{chapt:nef-extensions} widens the scope of the NEF, by characterizng the network-level effects of these higher-order synapse models, and harnessing them to implement certain classes of dynamical systems with improved accuracy.
Chapter~\ref{chapt:delays} explores a novel NEF-based computation that equips RNNs with an improved ability to compute nonlinear functions across windows of memory.
In this section, we provide the basic building blocks adapted from the original proposal by \citet{eliasmith2003a}, before analyzing its merits in Chapter~\ref{chapt:analysis}.

In the context of RNNs, the NEF offers a top-down means of training the weights of a recurrent network of spiking (or non-spiking) neurons to implement some particular dynamical system.
This is achieved efficiently through two important means: (1)~weight matrices are factorized into $n \times q$ (i.e.,~low-rank) \emph{encoding} and \emph{decoding} matrices, where $q$ is the dimensionality of the dynamical state-vector, and (2)~the optimization problem is reformulated to a simple batched least-squares problem that can be solved offline without needing to explicitly model the spike generation.
These insights lead to procedures for training and simulation that are efficiently implemented---scaling as $\bigoh{n}$ in time and space---by the Nengo~2.X simulator~\citep{bekolay2013}.
Backpropagation through time can be applied to improve the performance of these networks using Nengo~DL~\citep{hunsberger2015spiking, hunsberger2016training, hunsberger2018, rasmussen2018nengodl}.

At the same time, this top-down approach provides considerable transparency into the relationship between the neural nonlinearities and network function~\citep[][Chapter 7]{eliasmith2003a}.
In addition it becomes tractable to relate the time-constants of the low-level mechanisms to the emergent dynamics of the network.
The NEF consists of three mathematical principles used to describe neural computation.
...
We present this framework in a manner that is consistent with the  which implements the NEF among other approaches.
