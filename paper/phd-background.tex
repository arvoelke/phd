...
Assuming its mechanisms obey the laws of physics, it must also necessarily be a dynamical system~\citep{mckenna1994brain, port1995mind}.
...
It is instead necessary to constrain the solution space with some prior knowledge of the task.
...
The traditional approach to building an RNN is to take a single-layer feed-forward network, and recurrently connect the output of each neuron to all other neurons in the same layer with a square weight matrix.
...
This may optionally be stacked to form multiple layers, akin to a multi-layer feed-forward network where each layer is locally recurrent.

This approach also extends to more biologically-plausible realizations of the RNN, in which the continuous (i.e.,~real-valued) neural activations are replaced by spiking neurons that emit binary spikes over time, and weight matrices are replaced by synaptic filters that appropriately weight and exponentially decay incoming spikes over time.
...
These problems are intimately related to the ``credit-assignment problem'', in which a learner witnesses a combinatorial explosion of points in time to assign credit to for the current error; in the case of storing a single bit of information over time, the gradient is inherently unstable~\citep{bengio1994credit}.
Thus, a standard RNN trained by BPTT will often fail to learn tasks that require a longer term memory -- the precise use-case that RNNs were designed to cover in the first place.
...
To face the problem of learning across longer time-scales, additional constraints are often employed, such as those in the following section.

\subsection{Structured Approaches}

The Long Short-Term Memory~\citep[LSTM;][]{hochreiter1997long} network addresses the difficulty of storing information across long intervals of time by incorporating an explicit memory unit into the network.
The basic computational element is an LSTM ``cell'': a group of five nonlinearities and three multiplicative gates, linearly coupled to one another in a specific fashion, such that its default behaviour is to store its current input indefinitely by propagating an internal state variable back to itself on the next time-step (i.e.,~using an internal feedback connection).
Multiplicative gating controls whether the input should be integrated, whether the memory is to decay with some leakage, and whether the contents of the memory should be outputted~\citep{gers1999learning}.
Importantly, all of these mechanisms are differentiable, and so BPTT can be applied to learn all of these gating parameters automatically.
Thus, the LSTM can be viewed as a particular way of configuring and initializing an RNN in a manner that biases it towards latching onto values and persisting them for long, yet controlled, periods of time.

A spiking analog to LSTMs has also been proposed by \citet{bellec2018long}, in which adaptive neurons take on the role of an LSTM cell, since the adaptive state of the neuron is functionally similar to a controlled leaky integrator.
When BPTT is applied to such an SNN, they have shown it can approach the performance of a non-spiking LSTM network.
The structure of LSTMs have also been compared to that of cortical microcircuits in biology~\citep{costa2017cortical}.

In theory, the LSTM essentially circumvents the problems described in \citet{bengio1994learning} by restructuring the gradient that is needed to store bits of information.
The standard practice is to create layers of LSTM cells, and then stack several of these layers on top of one another, using weight matrices to connect each layer to the next~\citep{graves2013speech}.
This enhances the temporal complexity of the network's dynamics through a composition of time-scales, analogous to the manner in which deep feed-forward networks provide more complex internal representations via composition of spatial scales.
This has found much success in [examples], and has lead to a widespread adoption of LSTM networks as the RNN of choice (citation).
\TODO{Examples of when LSTMs fail?}

This has inspired several related efforts, including:
GRU, qRNN
