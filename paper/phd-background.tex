...
Studies involving power-usage are surprisingly scarse, but one such study demonstrated that scaling to one quarter of a million neurons required less than $30$\,W of power, in the worst-case~\citep{stromatias2013power}, or on the order of \numprint{e5}--\numprint{e6} times more power than a human brain of the same size.\footnote{
The human brain has \numprint{400000} times as many neurons, and uses $20$\,W of power.}
This is a significant improvement upon standard computing architectures, which devour at least \numprint{e8} times more power than a human brain~\citep{furber2012build}. 
However, power measurements are not easy to interpret or translate from one architecture to the next, as there are many other factors such as the total number of synaptic events, flexibility and scalability of the hardware, and open questions surrounding how much biological detail and accuracy is necessary for the target application~\citep{eliasmith2013build}--all of which make fair benchmarking an incredibly challenging problem~\citep{stewart2015closed}.
