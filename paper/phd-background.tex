...
However, this scheme applies only to \emph{linear} dynamical systems, relies on assumptions of near-instantaneous communication between all neurons, and requires linear neurons with minimal leakage and no refractory period~\citep{boerlin2013predictive}.
...
The NEF has been used to construct a wide variety of neural models, including a 6.6~million neuron functioning model of the human brain, dubbed ``Spaun'', that is capable of performing perceptual, motor, and cognitive tasks~\citep{eliasmith2012, choo2018}.
...
We interpret this observation pragmatically, as a call to: (1)~incorporate prior knowledge of a desired set of tasks into the network's construction, and (2)~constrain a network's function using available models of the underlying dynamical mechanisms.
However, the standard formulation of the NEF makes a number of simplifying assumptions regarding these mechanisms.
For example, the NEF typically assumes an exponential model of the post-synaptic current~(PSC) evoked by an action potential, which has a biologically implausible, instantaneous rise-time.
...
Chapter~\ref{chapt:nef-extensions} improves the scope of the NEF, by characterizng the network-level effects of these higher-order synapse models, and harnessing them to implement certain classes of dynamical systems with improved accuracy.
...
We find some connections between this system's NEF implementation and the neural response of ``time cells'' recorded in the rodent medial prefrontal cortex~(mPFC).
...
This is made efficient through two important steps: (1)~weight matrices are factorized into $n \times q$ (i.e.,~low-rank) \emph{encoding} and \emph{decoding} matrices, where $q \in \bigoh{1}$ is the dimensionality of the dynamical state-vector, and (2)~the optimization problem is recast as a batched least-squares problem that can be solved offline without needing to explicitly simulate the network over time.
...
Backpropagation through time can be applied to further improve the performance of these networks, by use of the Nengo~DL extension~\citep{hunsberger2015spiking, hunsberger2016training, hunsberger2018, rasmussen2018nengodl, blouw2018a}, which bidirectionally integrates Nengo with Google's popular deep learning toolkit, Tensorflow~\citep{abadi2016tensorflow}.

At the same time, this top-down approach provides considerable transparency into the relationships between neural nonlinearities, synapse models, their time-constants, and network function.
For instance, it becomes possible to determine the class of functions that are most accurately supported by a particular set of neurons~\citep[][Chapter 7]{eliasmith2003a}.
It also becomes tractable to relate the time-constants of the low-level mechanisms to the emergent dynamics of the network.
...
Its primary strengths rest in providing a well-understood and efficient means of engineering spiking neural models, and programming neuromorphic hardware, to perform mathematically described computations~\citep{eliasmith2013build, boahen2017neuromorph}.
We now provide an overview of these methods applied to training both feed-forward and recurrent connection weights, with an emphasis on implementing linear dynamical systems -- although these methods extend to nonlinear dynamical systems as well~\citep{voelker2017iscas, voelker2017neuromorphic}.
