...
That is, only the static ``readout'' weights to a linear output layer are modified, thus sidestepping the issues in BPTT.
This method and architecture are referred to as an Echo State Network~\citep[ESN;][]{jaeger2001echo}, or Liquid State Machine~\citep[LSM;][]{maass2002real}, depending on whether the neural nonlinearities are static or spiking models, respectively.
The intuition is that randomized feedback, when initialized in an appropriate manner, causes traces of the input to dynamically reverberate, or ``echo'', through the state of the network; a common metaphor is throwing rocks into a bucket of water and watching the ripples interact through time.\footnote{To demonstrate the relevance of this metaphor, \citet{fernando2003pattern} went so far as to actually implement an LSM using a bucket of water.}

There are two great insights driving the adoption of RC: (1) randomized feedback provides a suitable set of time-varying basis signals that can be linearly combined to approximate some output signal, and (2) by leaving the recurrent weights fixed, the optimization procedure is greatly simplified; training the readout weights amounts to a least-squares problem.
Additional details are provided in section~\ref{sec:relationships} after introducing the NEF.
In secion~\ref{sec:delay-rc}, we compare the performance of RC to the NEF on a delay-line benchmark.
