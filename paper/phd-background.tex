...
Rather, we exploit the fact that filtering and weighting spikes across the \emph{entire} population provides such an approximation, assuming that $n$ scales with $\tau^{-1}$.
...
In the NEF, individual neurons are free to spike much slower than ther ideal rate models, and slower than the time-constant of the filter (e.g.,~less than half the rate of its cutoff frequency).
