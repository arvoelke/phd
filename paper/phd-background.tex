...
This model maintains a one-dimensional voltage trace, $v_i(t)$, that continuously integrates the current-source $j_i(t)$ over time, with some leakage term governed by the time-constant $\tau_\text{RC}$ -- functionally equivalent to a resistor-capacitor~(RC) circuit, a first-order lowpass filter, or the convolution with an exponentially decaying impulse response.
...
In essense, equation~\ref{eq:encoding} defines a high-dimensional nonlinear projection of the vector $\V{x}(t)$, by taking its dot-product with an encoding matrix $E$, and providing the result as a current-source to generate $n$ heterogeneous spike-trains over time.
