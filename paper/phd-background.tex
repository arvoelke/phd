...
The NEF embraces all such methods, but makes a preference for those that are amenable to theoretical analysis while being useful and efficient in practice.
We find the approach of introducing random high-dimensional feedback to often be unnecessary for our purposes (section~\ref{sec:delay-rc}), and furthermore re-encoding the target output into the network as a teaching signal can lead to a loss in accuracy in simple cases (section~\ref{sec:force-comparison}).
...
%y(t) &= \V{d}^T \V{a}(t) \text{.}
%\end{align}
Intuitively, the reservoir expands the input into a set of nonlinear temporal features---referred to as {\it echoes}---while the readout combines these dynamical traces to approximate the desired target.
...
This is equivalent to the optimization performed by the NEF on the output transformation, but on data that is collected by explicitly simulating the network over time.
In contrast, the reservoir is untrained with dynamical properties that remain fixed, independently of the desired task.
...
Specifically, the recurrent weights include an additional outer-product term, $\V{e} \V{d}^T \in \mathbb{R}^{n \times n}$, where $\V{e} \in \mathbb{R}^n$ is a fixed random vector, and the vector $\V{d} \in \mathbb{R}^n$ are the same decoders from before.
By design, these weights decode the desired output, $y(t)$, and subsequently encode it back into the reservoir, alongside the random mixing from $W$.
...
In section~\ref{sec:force-comparison}, we show that this choice of re-encoding the output signal is sometimes not only suboptimal, but can reduce accuracy compared to an equivalent reservoir computer (i.e.,~without the additional FORCE-learned feedback).
...
A central observation made by the NEF is that the optimal $D$ can be determined from $\V{x}(t)$, the selection of neuron models, the models of postsynaptic current~(PSC), and whether the simulation is analog or digital.
...
In the same way as FORCE, the NEF may include $\V{d}$ as a column of $D$, and $\V{e}$ as a row of $E$, to re-encode $y(t)$ -- equivalent to asserting that a filtered version of $y(t)$ is a dimension of $\V{x}(t)$.
...
Now we shift focus to discuss the method(s) of training in some detail.
...
In general, the RC training method relies on \emph{explicit} simulation, since $\V{x}(t)$ is unknown. In the NEF the simulation is typically \emph{implicit} within Principle~2, but can also be made explicit in the same way (see Table~\ref{tab:learning-types}).
...
The NEF, by default, avoids explicit simulation because it is more costly (especially as the time-step $dt$ becomes small), which is of practical importance when engineering large-scale networks~\citep[e.g.,][]{eliasmith2012}.
However, the method does have the benefit of automatically refining the distortion error in the decoding induced by the substitution made by equation~\ref{eq:rates}.
...
On one hand, this can streamline construction of the network, while on the other it can mask costly mistakes being made by the modeller.
