...
$\delta(\cdot)$ is the Dirac delta, and $\left\{ t_{i\text{,}m} \right\}$ is the sequence of spike-times generated by the neuron model in response to the encoded signal. % via $G_i \left[ \cdot \right]$.
...
This model maintains a one-dimensional voltage trace, $v_i(t)$, that continuously integrates the current-source $j_i(t)$ over time, with some leakage governed by the time-constant $\tau_\text{RC}$ -- functionally equivalent to a resistor-capacitor~(RC) circuit, a first-order lowpass filter, or a convolution with an exponentially decaying impulse response.
...
\tau_\text{RC} \dot{v}_i(t) + v_i(t) = j_i(t) \text{,} \quad 0 \le v_i(t) < 1 \text{,}
\end{equation}
...
In essense, equation~\ref{eq:encoding} defines a high-dimensional nonlinear projection of the vector $\V{x}(t)$ over time, by taking its dot-product with an encoding matrix $E$, and providing the result as a current-source to $n$ heterogeneous spike-generators.

Having defined an encoding, we introduce a postsynaptic filter $h(t)$, which behaves as a model for the synapse by capturing the dynamics of a receiving neuron's current-sourcej
...
For now, we fix $h(t) = \frac{1}{\tau} e^{-\frac{t}{\tau}}$ to be an exponentially decaying PSC with time-constant $\tau$, which is equivalent (in the Laplace domain) to the canonical first-order lowpass filter (also known as a ``leaky integrator''), with the same dynamics as equation~\ref{eq:lif-model}:
\begin{equation}
\tau \dot{j}_i(t) + j_i(t) = \sum_{j} w_{ij} \delta \left( t - t_{j,m} \right) \text{.}
\end{equation}
