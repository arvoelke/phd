...
The perceptron is equivalent to a single-layer, feed-forward, neural network, where the input vector is multiplied by a matrix, and then projected through static nonlinear activation units (e.g., a threshold function, sigmoid, rectified linear, or similar).\footnote{A constant bias term is usually added to the weights, to control the activation threshold of each unit.}
...
Results on a chaotic time-series prediction benchmark suggest that LSTMs, when interleaved with our novel memory cell, and trained via BPTT, are capable of learning systems described by nonlinear delay-differential equations.
...
Such transparency enables theoretical insights into neural computation, and, as we will show, practical advances on RNN benchmarks that would otherwise be quite difficult to obtain with a pure black-box approach.
...
Its extension, full-FORCE~\citep{depasquale2018full}, can be viewed an an instance of the NEF, where training is performed online rather than offline, and randomized feedback is thrown in to serve the same purposes as in RC.
The methods of \citet{thalmeier2016learning} provide a means of realizing FORCE and full-FORCE networks in biologically plausible spiking networks using a precise spike-time coding mechanism, similar to ~\citet{boerlin2013predictive}, but supported by nonlinearities in the synapses.
...
In general, it is unclear how far these assumptions can be relaxed before scaling regresses to $\bigoh{\sqrt{n}}$, which is of great importance when considering our targets of modelling biological systems and mapping onto neuromorphic hardware -- both targets regularly violate these assumption.

Given the subtle distinctions between these similar sets of approaches,
section~\ref{sec:nef-suitability} analyzes the suitability of the NEF for universal spike-based computation on neuromorphic hardware.
