...
The term \emph{neuromorphic computing} refers broadly to the use of specialized hardware to emulate computational principles of biological nervous systems~\citep{mead1989analog, cohen2001report}.
...
Instead, neuromorphic architectures aim to implement specific classes of algorithms using dramatically less power, achieved by way of massively-parallelized event-driven computation that is sparse in both space and time~\citep{tang2017sparse}.
Such power savings are potentially tremendous, estimated to be on the order of a \numprint{e5}-fold increase in efficiency, which becomes necessary to reach the exascale level of computation performed by the human brain, which would otherwise require \numprint{e9}\ watts~\citep{furber2012build}.
However, improved power-efficiency often comes at the cost of reduced programmability and narrowed application spaces.
These are trade-offs that the community as a whole are still trying to navigate, and we make concrete advacements on with the formalisms in chapters~\ref{chapt:nef-extensions} and~\ref{chapt:delays}.

To make use of such hardware, we require significant advances in programming paradigms that can leverage its computational properties.
This involves inventing new algorithms, designing new frameworks and programming languages, and training the next generation of programmers to think differently about problems -- but the benefits are far-reaching: real-time computation that can in principle scale up to the cognitive, perceptual, and motor capabilities of the human brain, without requiring the energy produced by an entire nuclear power plant~\citep{furber2012build}.
As such, there are many common goals between neuromorphic hardware architectures, including: parallelism, memory colocated with computational elements, heterogeneity and sparsity in both time and space, minimization of spike traffic and synaptic events, and real-time data IO.
...
More generally, we provide a theoretical framework and extensions for understanding the class of computations that neuromorphic hardware enables, given some resource budget and desired precision.
...
Secondary goals include providing a balance between power, cost, and programmability, to make SpiNNaker accessible to a wide range of researchers in neuroscience and cognitive modelling communities.
...
Each core was prescribed to simulate \numprint{1000} spiking neurons and \numprint{1000000} synapses, although Nengo's integration doubled this target by achieving \numprint{2000} neurons per core through the use of factorized weight matrices~\citep{mundy2015}.
Each CMP is fully digital, locally synchronous, and clocked at some time-step.
The architecture scales up to \numprint{65536} CMPs---over one billion neurons and one trillion synapses---asynchronously connected to one another through a 2D triangular toroidal mesh.
An instantiation that realizes their initial target of 1\% of the human brain, using \numprint{1000000} cores, was finally reached as of 2018.

Scaling to one quarter of a million neurons required less than $30$\,W of power~\citep{stromatias2013power}, or on the order of \numprint{e6} times more power than a human brain of the same size.\footnote{
The human brain has \numprint{400000} times as many neurons, and uses $20$\,W of power.}
This is still a significant improvement upon standard computing architectures, which devour on the order of \numprint{e8} times more power than a human brain~\citep{furber2012build}. 
However, power measurements are not easy to interpret or translate from one architecture to the next, as there are many other factors such as the total number of synaptic events, flexibility of the hardware, and open questions surrounding how much biological detail is necessary for some target application~\citep{eliasmith2013build}.

Nengo has been ported to SpiNNaker~\citep{mundy2015} and used to run the full Spaun model in real-time~\citep{mundy2016real}.
This goal represented a \numprint{9000}-fold speedup over a conventional X86 CPU implementation~\citep{stewart2014large}.
...
\citet{liu2018memory} deploys a deep neural network on the prototype chip, realizing power improvements on the order of \numprint{e2} versus a traditional X86 CPU implementation.
