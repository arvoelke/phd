...
On the other hand, the human brain itself, assuming its mechanisms obey the laws of physics, must also necessarily be a dynamical system~\citep{mckenna1994brain, port1995mind}.
...
For tasks that involve sequential inputs, such as speech and language, RNNs are often the best choice~\citep{lecun2015deep}.
Some recent instances have demonstrated impressive human-like intelligence in generating captions from images~\citep{vinyals2015show}, recognizing speech in multiple languages~\citep{amodei2016deep}, and decoding human emotions from video~\citep{ebrahimi2015recurrent}.
...
The next largest (non-NEF) RNN that we can find with published numbers is a ``large scale'' speech recognition example from Google, which consists of \numprint{6800} neurons in its largest instantiation~\citep[][Table~1]{sak2014long}.
As we will see, the fundamental issue facing each method is the practical difficulty in scaling across time with larger networks.
