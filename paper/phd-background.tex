...
We begin with a very brief historical overview of many of the most popular architectural approaches and methods to building neurally-inspired computational systems, that are also dynamical in nature.
...
The perceptron is equivalent to a single-layer feed-forward neural network, where the input vector is multiplied by a learned weight-matrix, and then projected through static nonlinear activation units (e.g., a threshold function).
This static nonlinearity is intended to correspond to the behaviour of an artificial ``neuron'', and the matrix corresponds to the synaptic weights of each neuron (often including a bias term that controls the activation threshold).
There was a great deal of excitement spurred by Rosenblatt's claims that the perceptron would soon ``be able to walk, talk, see, write, reproduce itself and be conscious of its existence''~\citep{historyofperceptrons}.
...
The XOR function is perhaps the simplest example of a function that \emph{cannot} be computed by a perceptron.

Fortunately, multi-layer feed-forward networks do not suffer this severe limitation.
By repeating this process just once more, that is by stacking only two perceptrons together, the network becomes able to compute any bounded continuous function---a result known as the universal approximation theorem~\citep{hornik1991approximation}---assuming the nonlinear activation units are bounded, continuous, and non-constant.
This insight, together with the stacking of additional layers to compose increasingly-complicated functions, the use of ``backpropagation''~\citep{werbos1974beyond, rumelhart1986learning} to learn all of the weights from examples, when combined with increasingly-powerful CPUs and GPUs, all coalesced to usher in the era of ``deep learning''~\citep{lecun2015deep}.  
