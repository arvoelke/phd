...
An analagous insight in the above context is that a variety of transfer functions, when temporally composed, enhance the \emph{dynamical complexity} of network function.
...
As simple as this point might sound, a potent example is when one student, who had been working with the Nengo for many years, used an entire network to implement the same computation as a single alpha synapse. This was due to not having them described in the same language, thus highlighting the utility of the point that we are attempting to make here.}
