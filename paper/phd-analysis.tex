...
Specifically, the network is a one-dimensional communication channel ($f(x) = x$, $n = 100$, $\tau = 50$\,ms, $\dt{} = \numprint{e-5}$\,s) with maximum firing rates of $10$--$20$\,Hz and intercepts uniformly distributed across $[-1, -0.5)$.
...
We also duplicate the population, and replace its neuron model with the corresponding continuous-valued rate model given by equation~\ref{eq:lif-rate}.
...
Figure~\ref{fig:nef-error-types}(a) visualizes the static distortion in two different ways.
...
When a filter with a variance of $\bigoh{\tau^{-1}}$ in its impulse response---such as equation~\ref{eq:lowpass-impulse}---is applied to both the decoded spikes and the encoded input, then the error is scaled by $\tau^{-1}$.
On the other hand, the variability across the population leads to an increase in error by a factor of $\sqrt{n}$.
This latter factor follows from the convergence rate of the central limit theorem~\citep[CLT;][]{berry1941accuracy, esseen1942liapunov}, assuming the spiking noise is Gaussian and independent.
...
Figure~\ref{fig:nef-error-types}(b) visualizes a kernel density estimate~\citep[KDE;][]{michael_waskom_2015_19108} of this error ($t > 0.15$\,s).
...
In the same way that we derived the rate model of the LIF neuron~(see derivation leading to equation~\ref{eq:lif-rate}), we may derive the neuron's ISIP according to how far along it is within the ideal ISI normalized by its length, $r_i^{-1}$:
\begin{equation} \label{eq:isi-position}
\begin{aligned}
g_i &= 1 - \left( \rho_i + \tau_\text{RC} \ln \left( 1 + \frac{1 - v_i}{s_i - 1} \right) \right) r_i \\
...
\end{aligned}
\end{equation}
For example, if $\rho_i = \tau_\text{ref}$ and $v_i = 0$, then $g_i = 0$, since the neuron is at the very beginning of its ISI.
Likewise, if $\rho_i = 0$ and $v_i = 1$, then $g_i = 1$, as it has just spiked.
We now define the \emph{state discrepancy} to be the KS-statistic between the empirical ISIP distribution ($g_i$) and a uniform distribution over $\mathcal{U}[0, 1)$.

In Figure~\ref{fig:nef-error-types}(c), we measure this value empirically across all of neurons, and at every point in the simulation, using a window of $25$\,ms to smooth the estimate.
...
Although here we are looking at just a constant input for ease of visualization, we find that this statistic correlates highly with the spiking noise across a wide range of input frequencies (see Figure~\ref{fig:frequency-ks-test}).

We now prove why our definition of state discrepancy is indeed the correct one, and in doing so provide a sufficient (but not necessary) criteria for scaling away this source of error.
Specifically, this remaining source of error---state discprepancy---is tamed by minimizing the difference (KS-statistic) between a uniform distribution and the actual neuron's ``readiness to spike'', as characterized by its ISIP~(e.g.,~$g_i$ for the case of a LIF model).
