...
A proof is found in \citet[][pp.~132--136, appendix~C.1]{eliasmith2003a} that addresses the same issue from the perspective of including noise within the neuron model, and the contents herein may be viewed as an updated treatment of that study, focusing on the one-dimensional case for convenience.

Figure~\ref{fig:nef-error-types} illustrates all three types of error within the context of a single feed-forward simulation.
Specifically, the network is a one-dimensional communication channel ($f(x) = x$, $n = 100$, $\tau = 50$\,ms, $\dt{} = \numprint{e-5}$\,s, $\tau_\text{RC} = 20$\,ms, $\tau_\text{ref} = 2$\,ms) with maximum firing rates of $10$--$20$\,Hz and intercepts uniformly distributed across $[-1, -0.5)$.
Low firing rates are needed to bring the third source of error to the surface, which is normally masked by the other two at high firing rates for reasons that will become clear.
The population encodes a constant input signal of $u(t) = 1$, that is directly mapped onto the PSCs without additional filtering.
...
\begin{figure}
  ...
  \caption{ \label{fig:nef-error-types}
    ...
    (a)~A single simulation reveals all three types.
    ...
    (c)~State discrepancy provides an instantaneous measure of the difference between the spiking noise and a true Gaussian distribution ($p$-values~$< \numprint{e-6}$).
...
Specifically, for a given $f$, a linear increase in the number of basis functions will reduce the root-mean-squared error~(RMSE) by the same factor.
This RMSE is named \emph{static distortion}, since it corresponds to the difference between the two static (i.e.,~non-temporal) functions.
We commonly refer to the RMSE as simply the ``error'' throughout,\footnote{
\citet{eliasmith2003a} uses ``error'' to refer to mean-squared error~(MSE), rather than the RMSE. This may lead to confusion when comparing across sources.}
and define ``precision'' to be the reciprocal of error.
Thus, we say that the precision of static distortion, or a non-spiking network, scales as $\bigoh{n}$.
...
The $\sqrt{n}^{-1}$ term follows from the convergence rate of the central limit theorem~\citep[CLT;][]{berry1941accuracy, esseen1942liapunov}, assuming the spiking noise is Gaussian and independent.
...
We define a neuron's relative ``ISI position''~(ISIP) to be its normalized position within the inter-spike interval that corresponds to the instantaneous firing rate of the ideal rate model.
...
We now define the \emph{state discrepancy}, at some chosen moment in time, to be the KS-statistic between the empirical ISIP distribution across the population, $\{g_i(v_i, \rho_i) \, : \, i = 1 \ldots n\}$, and a uniform distribution over $\mathcal{U}[0, 1]$.
...
Let $\V{\eta}(t) = \sum_{i=1}^n \left(a_i \ast h \right)(t) \V{d}^\V{f}_i- \sum_{i=1}^n \left( r_i(\V{x}(t)) \ast h \right)(t) \V{d}^\V{f}_i$
% \transpose{\V{d}}a - \hat{x}$
be the spiking noise.
If the ISIP is uniformly and independently distributed, as in $g_i \sim \mathcal{U}[0, 1]$, then:
\begin{enumerate}
\item $\expect{ \V{\eta}(t) } = 0$; hence the expected spike noise is exactly zero at all times (no systematic bias), 
\item $\sqrt{\variance{\V{\eta}(t)}} = \bigoh{\left(\tau \sqrt{n}\right)^{-1}}$ is the standard error, or root-mean-squared error; hence the precision scales as $\bigoh{ \tau \sqrt{n} }$ at all times, and therefore:
\item equation~\ref{eq:rate-approximation} holds for the noise term $\V{\eta}(\cdot) \sim \mathcal{N}\left(0, \sigma^2 \right)$, where $\sigma = \bigoh{ \left( \tau \sqrt{n} \right)^{-1} }$.
\end{enumerate}
\end{theorem}
We prove each statement in turn.
For the first two statements, we prove something slightly more general, and without invoking asymptotics until the very end.
We also shift our frame of reference, without loss of generality, such that we are currently observing $t = 0$.
...
This is because the statements in this theorem are only interested in taking a measurement at arbitrarily small values of $t$.
...
In order to make this concrete, we suppose the synapse model is equation~\ref{eq:lowpass-impulse}, but this is not a hard requirement.
Likewise, we initialize the synapse to $0$, which serves only to simplify the discussion.
To remove boilerplate in the notation we substitute $r_i \leftarrow r_i(\V{x}(t))$.
We have verified the following equations with rigorous numerical experiments~(not shown).

\subsubsection{Preliminaries}

Let $\tilde{y}_i(t; t_i) = \left(a_i \ast h \right)(t)$ be the foreward simulation of the $i^\text{th}$ neuron up to time $t \ge 0$, where $t_i = \left(1 - g_i\right)r_i^{-1}$ is the next spike-time ($0 \le t_i \le r_i^{-1}$) by our definition of the ISIP.
...
f_{T_i}(t_i) = r_i^{-1} \quad \iff \quad P(T_i \le t_i) = t_i r_i \text{.}
\end{equation}
...
\begin{equation}
\begin{aligned}
\expect{\V{\eta}(t)} &= \expect{\sum_{i=1}^n \left(a_i \ast h \right)(t) \V{d}^\V{f}_i - \sum_{i=1}^n \left( r_i \ast h \right)(t) \V{d}^\V{f}_i } \\
...
However, to obtain a closed-form solution for the variance, we will need to assume that $T_i$ is independently distributed (e.g.,~the voltage vectors and refractory periods are not coupled with one another), as is the case for typical NEF networks.
\citet{boerlin2013predictive} have shown that if the neural states do have a mechanism for collaborating in a global manner, then they can synchronize their activity, as a way of \emph{dependently} sampling, in order to reduce variance.
...
Finally we use equation~\ref{eq:a_expectation} in the second step, which lets allows us to leverage the property that for two independent \emph{zero-mean} variables, $A$ and $B$, we have $\variance{AB} = \variance{A}\variance{B}$.
Therefore,
\begin{equation}
\begin{aligned}
\sqrt{ \variance{ \V{\eta}(t) } } &= \sqrt{ \variance{\sum_{i=1}^n \left(a_i \ast h \right)(t) \V{d}^\V{f}_i- \sum_{i=1}^n \left( r_i \ast h \right)(t) \V{d}^\V{f}_i} } \\
...
\end{aligned}
\end{equation}
This completes our proof of the second statement.
The third statement holds by equation~\ref{eq:rate-approximation} for large $n$ by the central limit theorem and the first two moments of $\eta(t)$.
