...
\citet{gerstner1999spiking} observes that there are at least three different ways to define a rate code, and that in may important ways they are consistent with that of a timing-based code.
...
\citet{brette2015philosophy} argues that the question about which code the brain uses is irrelevant, and should be replaced by one that addresses the causal role of neural activity.
...
\begin{figure}
    ...
    \caption{\label{fig:spike-time-coding} 
      ...
      We propose this be understood as a ``postsynaptic current code'' instead.
...
Our explanation for this seemingly paradoxical observation is resolved by the creation of an entirely different label: a ``postsynaptic current code''.
This code does not care about the spike-rates of individual neurons; it is only sensitive to how well the weighted and synaptically-filtered spikes, when pooled across the entire population, approximate some desired set of postsynaptic currents (corresponding to an affine transformation of the required state-vector)~\citep{tripp2006neural}.
The subtleties surrounding how this works and why this distinction can matter is often lost, even amongst experts in the field, and thus these discussions sometimes regress to harmful statements such as ``it is okay; I know the NEF is sending rates''~[personal communication].
But as our example illustrates, the NEF is not adhering to any particular definition of rate or timing code.
