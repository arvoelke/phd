...
The primary goal of this thesis is to unpack the above statements through the exploration of fundamental problems and their theoretical solutions, validated by practical results that include functional SNNs deployed on state-of-the-art neuromorphic chips: Braindrop and Loihi.
...
Theoretical frameworks help guide decisions in hardware design, while hardware constraints determine relevant extensions to theoretical frameworks -- both processes coexist in a dynamic feedback loop.
...
its Python package, Nengo, and its novel extensions detailed within this thesis.
...
However, we do not believe that deeper networks, better training heuristics, or larger datasets, will be enough to achieve our aforementioned goal.
...
But at large, most ANNs do not begin with {\it time} as a physical dimension intrinsic to the dynamics of the system's input, output, or processing units; time is usually incorporated later at another level of abstraction.
...
If our aim is to build artificial system that are as robust, capable, and scalable as the human brain, then we should strive to understand computation in such terms.
