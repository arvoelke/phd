...
\begin{figure}
    \centering
    \includegraphics[width=0.5\textwidth]{touch-1}
...
\end{tikzpicture}
\caption{\label{fig:decoding-filter} Architecture used to formulate the optimization problem that simultaneously solves for the decoding weights and the optimal linear filter.}
\end{figure}
...
\item Extend the NEF with respect to this new model specificiation.
\end{enumerate}
...
\end{enumerate}
Integrate-and-fire (i.e.,~ReLU) spiking neurons also share benefits 3, 4, and 5 -- assuming its voltages are initialized to $v_i(0) \sim \mathcal{U}[0, 1]$ at the beginning of the simulation.
...
\caption{\label{fig:poisson-frequency-scaling} Performance of spiking rectified linear (ReLU), Poisson-spiking LIF, and regular-spiking LIF, as the frequency of the input signal is scaled.
...
Due to the memoryless property of the Poisson process, and the uniformity of the ReLU, both outperform LIF with constant precision as $f \rightarrow \infty$ (Theorem~\ref{thm:correctness}), assuming $n$ and $\tau$ are scaled appropriately with frequency (see text for details).
}

\vspace{1em}

\includegraphics[width=0.9\textwidth]{poisson-neuron-scaling}
\caption{\label{fig:poisson-neuron-scaling} Performance of spiking rectified linear (ReLU), Poisson-spiking LIF, and regular-spiking LIF, as the number of neurons are scaled.
...
Spiking ReLU and Poisson-spiking LIF performance scale towards $0$ error as $n \rightarrow \infty$ (Theorem~\ref{thm:correctness}), since each neuron is always ready to spike with the ideal probability. 
}
\end{figure}

We validate these claims in Figures~\ref{fig:poisson-frequency-scaling} and~\ref{fig:poisson-neuron-scaling} by using a Poisson-spiking version of the LIF model, evaluating the root-mean-squared error (RMSE) against the filtered ideal, and comparing to both the spiking ReLU and the regular-spiking LIF.
...
In Figure~\ref{fig:poisson-frequency-scaling}, we demonstrate that precision remains constant for both Poisson and ReLU neurons, even at frequencies that more than double the maximum firing rates of any given neuron.
...
In Figure~\ref{fig:poisson-neuron-scaling}, we demonstrate that the precision continues to scale as $\bigoh{\tau \sqrt{n}}$ for the Poisson LIF and spiking ReLU, but not for LIF.
