...
\TODO{Mention that FORCE and full-FORCE don't do any better (not shown)}
...
\subsection{Cyclic Pattern Detection}
\label{sec:periodicity}

Given an arbitrary scalar signal, $u(t)$, we define the measure of \emph{$k$-periodicity} for natural $k \in \mathbb{N}_{\ge 1}$, with respect to some finite interval length $\theta > 0$, and sub-interval length $\gamma = \frac{\theta}{k}$, as follows:
...
Lower values of $p_k(t)$ imply deconstructive interefence between the superposition of each segment, and thus we refer to such signals as being \emph{aperiodic}.
...
This has applications as a general low-power signal processing tool, and for modelling how cyclic patterns could be detected by neural mechanisms in a robust and flexible manner.
...
\end{equation}
where $(\cdot)^+$ denotes the Moore-Penrose pseudoinverse, $B(\cdot, \cdot) \in \mathbb{R}^{m \times q}$ is a matrix corresponding to a slice of the realized basis function~\ref{eq:basis-functions}, as in:
\begin{equation}
B(a, b) = \begin{pmatrix}
...
Since the buffered window of input history is a linear transformation of the DN's state vector, we start by considering how $u \left( t - \theta + i \gamma + \tau \right)$ can be obtained from $\V{x}(t)$. 
...
Since the ideal computation involves a linear transformation of the input window, it can be recast as a linear transformation of the state.
