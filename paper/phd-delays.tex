...
\end{equation}
For the special case of $q = 1$, this reduces to the first-order lowpass $\theta \dot{x}(t) + x(t) = u(t)$, as in equation~\ref{eq:lowpass-laplace} ($\tau \leftarrow \theta$).
For $q > 1$, the $q^\text{th}$ dimension recursively extends the system for $q - 1$ to represent a $q^\text{th}$-order polynomial of history.
...
\fig{legendre-circuit-diagram}{1.0}{Scalable circuit diagram representation of the Legendre Delay Network.}{
    Circuit diagram that represents an efficient implementation of the Legendre realization of the Delay Network ($q = 6$).  %, discretized using Euler's method.
    Large circles correspond to each dimension of $\V{x}$. 
    Small circles indicate elements that add (arrow head) or subtract (circular head) their inputs.
    The $i^\text{th}$ dimension scales its input (triangular head) by $\dt{} (2i + 1) \theta^{-1}$, for $i = 0 \ldots q - 1$.
    Recurrent self-loops, from each dimension back to itself, are omitted for simplicity.
}
...
Increasing the dimensionality of the system by one requires appending $\bigoh{1}$ wires, adders, and state-variables, to the existing circuitry.
In total, this circuit requires $\bigoh{q}$ wires, adders, and state-variables, thus making it linearly scalable in both space and time (see section~\ref{sec:delay-scalability}).
...
\fig{legendre-delay}{1.0}{\numprint{10240}-dimensional simulation of the Delay Network.}{
    Scaling the Delay Network to $q = \numprint{10240}$ dimensions, simulated for $\numprint{1024000000}$ time-steps ($\theta = \numprint{5120}$\,s, $\dt{} = \numprint{e-5}$\,s).
    The input signal is $u(t) = 1$, and so $y(t) \approx u(t - \theta)$ is the Heavide step function centered at $t = \theta$.
    Each state-variable approximates the integral of a Legendre polynomial, and $y(t) = \sum_i x_i(t)$ sums across all dimensions at each moment in time.
}

Figure~\ref{fig:legendre-delay} reveals that the delay network scales to over $\numprint{e4}$ dimensions to efficiently maintain information across a simulation lasting more than $\numprint{e9}$ time-steps.
This is demonstrated by implementing the design from Figure~\ref{fig:legendre-circuit-diagram} in C++, which takes $\bigoh{q}$ time and memory per simulated time-step using a traditional CPU.
...
Lastly, when discretizing the system in time (e.g.,~ZOH) or using a non-canonical realization (e.g.,~balanced), larger values of $q$ require denser interconnectivity ($q \times q$ transform matrices) between all $q$ dimensions, leading to $\bigoh{q^2}$ time and memory requirements.
...
\newpage % \TODO{Check that this improves aesthetics.}
...
\end{aligned}
\end{equation}
We remind the reader that $A$ and $B$ may be transformed by any of the state-space realizations from section~\ref{sec:software}, or starting with the Legendre realization from section~\ref{sec:legendre-basis}, before proceeding with the remaining numerical machinery.
...
\begin{figure}
  ...
  \caption[Comparison of time cells to a Delay Network.]{ \label{fig:time-cells}
    ...
    (Top)~Spiking activity from the rodent mPFC~\citep[reproduced from][Figures~4C and~4D, by permission of Oxford University Press]{tiganj2016sequential}.
