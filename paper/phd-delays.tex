...
\fig{pade-delay-error}{1}{Performance scaling of the Deep Delay Network.}{
...
\fig{force-comparison}{0.7}{Memory capacity of FORCE versus the NEF.}{
    ...
    Each network is trained to delay a time-varying input by $\theta$ seconds.
    The FORCE networks consists of $500$ $\tanh$ neurons, while the NEF network is $6$ linear neurons.
    See text for details.
}

\TODO{Rerun with lower dt and higher learning rate again.}
...
%This reveals a situation in which training the network to re-encode its target output can hinder performance, relative to .
%The NEF solution proposes a means of understanding this phenomenon.
%In particular, the target output is only one dimension among six other orthogonal dimensions that must all be encoded into the state of the network.
