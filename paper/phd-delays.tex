...
\begin{equation} \label{eq:delay-system}
  ...
  B = \transpose{\begin{pmatrix} v_0 & 0 & \cdots & 0\end{pmatrix}} \text{,} 
\end{equation}
where $v_i := (d+i)(d-i)(i+1)^{-1}$ no longer depends on $\theta$. %and $w_i := (-1)^{d - 1 - i} \left( \frac{i+1}{d} \right)$
...
In other words, the network is representing a rolling window (i.e.,~history) of length $\theta$, or a temporal code of the input stimulus.
