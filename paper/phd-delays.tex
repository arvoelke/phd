...
However, after decreasing the time-step, by an order of magnitude, to $\dt{} = 200$\,$\mu$s---while increasing the length of data by the same factor so that the data still represents the exact same signals---the performance collapses; accuracy exponentially decays as a function of delay length across the $\numprint{1000}$ time-step window (see Figure~\ref{fig:lstm-capacity}~(b)).
