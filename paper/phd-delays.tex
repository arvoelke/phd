...
\TODO{Takens Theorem. Mental time-travel. INRC delay motivation slide. Class of dynamical-systems-based algorithms.}
...
In this section we address several fundamental questions regarding the scalability of the Delay Network with respect to $q$, $\theta$, and the complex frequency of the input signal, $s = 2 \pi i f$, for some real frequency, $f$.
...
Next, as shown in Fig.~\ref{fig:pade-delay-error}, the radius of convergence for the Pad\'e approximants increases linearly with $q$.\footnote{
...
\begin{figure}
    ...
    \caption{\label{fig:comm-channels} 
      ...
      (Top)~A standard feed-forward communication channel, implemented by decoding the identity funciton at each layer.
      (Bottom)~A recurrent communication channel, implemented by a Deep Delay Network~(DNN) approximating $u(t)$ at each layer. Each layer is a DN with $\theta = 50$\,ms and $q = 6$.
...
We now consider an architecture that stacks multiple DNs on top of one another, to form a \emph{Deep Delay Network}~(DNN) chaining multiple delays together.
...
\begin{figure}
  ...
  \caption{\label{fig:deep-delay-network-error}
    Visualizing the error in the Deep Delay Network~(DNN) of width~$q$ and depth~$k$ (see equation~\ref{eq:deep-delay-network-error}) for two different resource-cost functions.
