...
We compare all four networks by sweeping $\theta$ across $0.01$--$1$\,s, with $10$ trials at each value of $\theta$ (boostrapped 95\% confidence intervals).
...
\fig{force-comparison}{0.7}{Memory capacity of FORCE versus the NEF.}{
    ...
    Network are trained to delay a time-varying input by $\theta$ seconds.
...
we use an off-the-shelf implementation of a stacked LSTM~\citep[Keras;][]{gulli2017deep}
to construct $k=3$ layers with $n=50$ cells each.
...
The input data is white noise, band-limited to $30$\,Hz, starting at $0$, and normalized to an absolute range of $[-1, 1]$.
...
The training data is further partitioned into a different random $25$\% sample used for validation at every epoch.
Backpropagation through time is carried out by the Adam optimizer~\citep{kingma2014adam} with respect to the MSE loss function.
Training is parallelized using TensorFlow~\citep{abadi2016tensorflow} across four Nvidia Titan Xp GPUs (12\,GB each).\footnote{%
...
We found that for time-steps ($\dt{}$) on the order of a couple milliseconds or more, backpropagation could find adequate parameters to solve this task -- that is, the LSTM could in fact represent the entire delay interval consisting of about $100$ time-steps (not shown).
However, after decreasing the time-step, by an order of magnitude, to $\dt{} = 200$\,$\mu$s---while increasing the length of data by the same factor so that the data still represents the exact same signals---the performance collapses; accuracy exponentially as a function of delay length across the $\numprint{1000}$ time-step window (see Figure~\ref{fig:lstm-capacity}~(b)).
...
\begin{figure}
  ...
  \begin{subfigure}{.5\textwidth}
    ...
    \caption{Testing}
  \end{subfigure}
  \caption[Memory capacity of stacked LSTMs versus Delay Network.]{
    ...
    but grows exponentially for LSTMs (linear on a log-scale) up to a maximum $100$\% error.
    See text for details.
  }\label{fig:lstm-capacity}
\end{figure}
...
We see that the DN architecture achieves nearly uniform accuracy across the delay interval, while the equivalently-sized LSTM cell archictecture approaches $100$\% error rates towards the end of the window.
...
On the other hand, this task is natural for the stacked DN, as its state represents a $q$-degree polynomial expansion of input history (see Figure~\ref{fig:basis-functions}).
We now proceed to demonstrate that backpropagation enables the stacked DN cells to outperform the stacked LSTM even on tasks that are not readily supported by the initial configuration of the network.
...
%This motivates the discovery of a new cell that we derive using . An accurate spiking implementation of this cell in Nengo displays a striking resemblance to time cells in the cortex. Applying backpropagation through time enables an equivalently-sized network to store longer windows of time, train faster, and outperform a stacked LSTM on the Mackey-Glass dataset.
