...
Our first set of experiments are designed to evaluate the memory capacity of stacked LSTMs relative to stacked Delay Networks of equivalent resource suage.
...
We set $q=6$ and initialized $\bar{\theta} \in \mathcal{U}[25, 50]$ to account for the shorter time-scale of this dataset.
...
The last step is generally infeasible due to the chaotic nature of the MG system; any error in the state estimate is exponentially magnified over time (the butterfly effect; also see section~\ref{sec:chaos}).
This makes a look-ahead of $15$ steps particulary challenging for a relatively small network, as its reconstruction of the chaotic manifold must become exponentially more precise to see linear payoffs in accuracy.
